...
class MLP(torch.nn.Sequential):
    def __init__(self, input_dim, output_dim, hidden_dims_lst):
        ...
        self.norm = torch.nn.ModuleList(
            [
                torch.nn.BatchNorm1d(num_features=dims[i + 1], affine=False)
...
class CNN(torch.nn.Sequential):
    def __init__(
        ...
        if normalization == "batch":
            self.norm = torch.nn.ModuleList(
                [
                    torch.nn.BatchNorm1d(num_features=in_ch[i + 1], affine=affine)
            ...
            self.norm = torch.nn.ModuleList(
                [
                    torch.nn.InstanceNorm1d(num_features=in_ch[i + 1], affine=affine)
                    for i in range(self.layer_size)
                ]
            )
            self.norm = self.norm.double()
...
class CNN_RNN(torch.nn.Sequential):
    def __init__(
        ...
        self.fc1 = torch.nn.Linear(RNN_hidden_dim * direction * n_size_p, hidden_dim)
    ...
    def forward(self, v):
        ...
        if self.Use_GRU_LSTM == "LSTM":
            ...
            h0 = torch.randn(
                self.RNN_layers * direction, batch_size, self.RNN_hidden_dim
            ).to(self.device)
            c0 = torch.randn(
                self.RNN_layers * direction, batch_size, self.RNN_hidden_dim
            ).to(self.device)
